...
void createVideoImage(const AVFrame* frame, AVCodecContext* pCodecContext, VideoFrameImage* image)
{
	...
	ImageDataFormat glFormat = ImageDataFormat::RGB;
	...
	image->init(glFormat, destPixels, width, height, nUnitSize);
...
void VideoFrameImage::init(ImageDataFormat format, uint8_t* pixels, uint32_t width, uint32_t height, int nUnitSize)
{
...
bool MediaFFmpeg::load(const std::string& filepath)
{
	this->loadFFM(filepath);
...
void MediaFFmpeg::setVideoFrame(float frame)
{
...
void MediaFFmpeg::loadFFM(const std::string& filepath)
{
	if (!s_bInitFFmpeg)
	{
		av_register_all();
	...
	do
	{
		if (loadFromLocalFile(filepath))
		{
			break;
		}
		if (loadFromRemoteUrl(filepath))
		{
			break;
		}
...
bool MediaFFmpeg::loadFromLocalFile(const std::string& fullpath)
{
	if (fullpath.empty())
	{
		return false;
	...
	if (avformat_open_input(&_formatContext, fullpath.c_str(), 0, nullptr) < 0)
	{
		return false;
	}
...
bool MediaFFmpeg::loadFromRemoteUrl(const std::string& path)
{
	...
	Directory::createFile(localFilePath);

	ConcurrentFile file(localFilePath);
