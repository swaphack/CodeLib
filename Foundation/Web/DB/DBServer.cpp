...
DBServer::DBServer(sys::IDataBase* db)
{
	_db = db;
}
...
bool DBServer::load(const std::vector<std::string>& readTableNames, std::map<std::string, sys::DataSheet*>& tableSheet)
{
...
bool DBServer::insert(const char* tableName, const std::map<std::string, std::string>& values)
{
...
bool DBServer::del(const char* tableName, const std::map<std::string, std::string>& conditions)
{
...
bool DBServer::update(const char* tableName, const std::map<std::string, std::string>& conditions, const std::map<std::string, std::string>& values)
{
	if (_db == nullptr || tableName == nullptr || values.size() == 0)
	{
		return false;
...
bool DBServer::exec(const char* strSQL, sys::IDataSheet* pDataSheet)
{
